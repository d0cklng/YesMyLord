...
void JinRandom::SeedMT( uint32_t seed )
{
    printf("JinRandom seed=%i\n",seed);
...
uint32_t JinRandom::ReloadMT( void )
{
...
uint32_t JinRandom::RandomMT( void )
{
    uint32_t y;
...
float JinRandom::FrandomMT( void )
{
    return ( float ) ( ( double ) RandomMT() / 4294967296.0 );
}

void JinRandom::FillBufferMT( void *buffer, uint32_t bytes )
{
    uint32_t offset=0;
    uint32_t r;
...
uint32_t JinRandom::rnd()
{  //只要随机数,不管细节的办法.
    static JinRandom sRand;
    static bool sRandInited = false;
    if(!sRandInited)
    {
        sRand.SeedMT(JinDateTime::getTime(NULL));
        sRandInited = true;
    }
    return sRand.RandomMT();
}
