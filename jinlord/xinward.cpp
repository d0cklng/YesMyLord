...
XInward::XInward(const JinNetAddr &use4realTarget,
             const JinString &selfIdentify)
    : xlorb_(NULL)
    , selfIdentity_(selfIdentify)
    , isStart_(false)
{
    forTarAddr_ = use4realTarget;
...
bool XInward::start(IXLorb* xlorb)
{
...
void XInward::DoCloseAdmitID(ClientInfo *info, const NetID &id)
{
    JAssert(info->id == id);
...
void XInward::DoRecvUnderFlow(XLorsSocket *sock, CallbackInfo info)
{
...
void XInward::sHandleTimeout(TimerID id, void *user, uint64_t user64)
{
    XInward* lorb = (XInward*)user;
    lorb->handleTimeout(id,(ClientInfo*)user64);
}

void XInward::handleTimeout(TimerID id, ClientInfo* info)
{
...
//ISinkForLorsSocket
void XInward::OnConnect(XLorsSocket *sock, CallbackInfo info, bool success, const JinNetAddr &addr, const JinNetPort &port)
{
    ...
    xlorb_->DoSend((NetPacket*)joiner.buff(),joiner.length()-1, info->id, tunnid);
...
//ISinkForLorsSocket
void XInward::OnRecv(XLorsSocket *sock, CallbackInfo info,const char *buf, uint32_t byteRecved)
{
#ifdef JDEBUG
    if(byteRecved<=32)
    {
    ...
    xlorb_->DoSend((NetPacket*)joiner.buff(),joiner.length()-1, info->id,
...
//ISinkForLorsSocket
void XInward::OnDisconnect(XLorsSocket *sock, CallbackInfo info, bool byRemote)
{
...
//原来是ISinkForOuterAdmit的,现在是public函数直接被调用.
bool XInward::OnRecv(const NetID &id, const JinNetAddr &addr, const JinNetPort &port,
...
//ISinkForOuterAdmit
bool XInward::OnIncoming(const NetID &id, const JinNetAddr &addr, const JinNetPort &port)
{
...
//ISinkForOuterAdmit
void XInward::OnClose(const NetID &id, bool grace)
{
...
bool XInward::OnAdmitDataTransfer(ClientInfo *info, JinPackParser *parser, size_t len)
{
...
bool XInward::ReplyAckTo(ClientInfo *info, uint64_t ack)
{
    JinPackJoiner joiner;
    joiner.setHeadID(kMainA2F,kAsstFlowCtrl);
    joiner.pushNumber(kUkAck,ack);

    return xlorb_->DoSend((NetPacket*)joiner.buff(),joiner.length()-1, info->id);
}

bool XInward::OnAdmitOpenTo(ClientInfo *info, JinPackParser *parser)
{
    ...
    if(forTarAddr_.isValid())  lsock->bind(forTarAddr_,0);
...
bool XInward::ReplyOpenTo(ClientInfo* info, uint16_t tunnid, XlorsErrorCode ec, const char* eMsg)
{
    ...
    return xlorb_->DoSend((NetPacket*)joiner.buff(),joiner.length()-1, info->id, tunnid);
}

bool XInward::OnAdmitCloseTunnel(ClientInfo *info, JinPackParser *parser)
{
    InfoMoreType& more = info->more;
    if(more == NULL || more->tunnSock == NULL){
...
bool XInward::OnAdmitGreeting(ClientInfo *info, JinPackParser *parser)
{
    InfoMoreType& more = info->more;
    if(more == NULL)
    {
...
bool XInward::ReplyGreeting(ClientInfo *info, char cmpChr)
{
    JinPackJoiner joiner;
...
bool XInward::OnAdmitFlowCtrl(ClientInfo *info, JinPackParser *parser)
{
    InfoMoreType& more = info->more;
    if(more == NULL || more->tunnSock == NULL){
        JDLog("no more, no more tunn sock. more=%p",more);
